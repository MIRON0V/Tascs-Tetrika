SELECT ride_date, user_id, MIN(average_score)

FROM (SELECT date(scheduled_time) AS ride_date, user_id, ROUND((SUM(grade)/COUNT(user_id)),2) AS average_score
      FROM  (SELECT user_id, role, event_id
            ...
            (SELECT lesson_id, (tech_quality / COUNT(lesson_id)) AS grade 
             ...
             ORDER BY ride_date, average_score)query_in 
